...
public class HauptFensterController {
	...
	private void initialize() {
		...
		// tbl_kompetenzen_Name.setCellValueFactory(cellData ->
		// cellData.getValue().nameProperty());
		// tbl_phasen_name.setCellValueFactory(cellData ->
		// cellData.getValue().nameProperty());
		// tbl_phasen_startDatum.setCellValueFactory(cellData ->
		// cellData.getValue().startDateProperty());
		// tbl_phasen_endDatum.setCellValueFactory(cellData ->
		// cellData.getValue().endDateProperty());
		// kompetenzenData =
		// FXCollections.observableArrayList(projekt.getKompetenzen());
		...
		// indexKompetenz =
		// tbl_kompetenzen.getSelectionModel().getFocusedIndex();
		// indexKompetenzClicked = true;
		// if(indexKompetenzClicked && indexPhaseClicked &&
		// !kompetenzenData.isEmpty() && !phasenData.isEmpty()){
		//
		// int indexIntern = 0;
		// int indexExtern = 0;
		// int i = 0;
		// for (Aufwand aufwand :
		// projekt.getPhasen().get(indexPhase).getAufwände()) {
			...
			public void handle(MouseEvent mouseEvent) {
				...
				if (indexKompetenzClicked && indexPhaseClicked && !kompetenzenData.isEmpty() && !phasenData.isEmpty()) {
					...
					for (Aufwand aufwand : projekt.getPhasen().get(indexPhase).getAufwände()) {
						if (aufwand.getZugehoerigkeit()
								.equals(projekt.getKompetenzen().get(indexKompetenz).getName())) {
							if (aufwand.getName().equals("Intern"))
								indexIntern = i;
							else
								indexExtern = i;
					...
					txt_ptIntern.setText(
							String.valueOf(projekt.getPhasen().get(indexPhase).getAufwände().get(indexIntern).getPt()));
					txt_ptExtern.setText(
							String.valueOf(projekt.getPhasen().get(indexPhase).getAufwände().get(indexExtern).getPt()));
				}
			}
		});
	}
